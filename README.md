...
| 🎲 [Пустой тессеракт капсулы](docs/patches/ancient-mega-pack-void-capsule.md) | 📦 [Ancient Mega Pack](#затронутые-моды) | 🧪 TEST: исправляет некорректное пустое значение `Unique` у всех предметов капсульных пулов |
