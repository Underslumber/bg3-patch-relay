...
Краткое и полное описания страницы mod.io хранятся в `docs/modio-profile.json`. Ручной запуск с `sync_profile=true` записывает их через авторизованный браузер выделенной VM и проверяет результат на публичной странице; новый файл мода при этом не собирается и не публикуется.
